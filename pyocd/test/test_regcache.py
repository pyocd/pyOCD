...
class TestRegisterCache:
    ...
    def test_read_cached(self, mockcore, regcache):
        self.set_core_regs(mockcore)
        # cache all regs
        regcache.read_core_registers_raw(CORE_REGS_NO_CFBP.values())
    ...
    def test_write_regs(self, mockcore, regcache):
        self.set_core_regs(mockcore)
        for r in CORE_REGS_NO_CFBP:
            regcache.write_core_registers_raw([r], [get_expected_reg_value(r) + 7])
        for r in CORE_REGS_NO_CFBP:
            assert mockcore.read_core_registers_raw([r]) == [get_expected_reg_value(r) + 7]
