...
"""HPM68x0 (Y=0 base) part target definition for pyOCD.

HPM6800 family Y=0 base target (HPM6830/6850/6880).
Single-core RISC-V MCU. No internal flash.

Memory map (no flash):
    ILM:         0x00000000, 256KB (rwx)
    DLM:         0x00080000, 256KB (rw)
    AXI_SRAM:    0x01200000, 512KB (rwx)
    AHB_SRAM:    0xF0400000, 32KB  (rw)
    APB_SRAM:    0xF4130000, 8KB   (rw)
"""
...
_AHB_SRAM_BASE = 0xF0400000
_AHB_SRAM_SIZE = 0x8000       # 32 KB
_APB_SRAM_BASE = 0xF4130000
...
class HPM68x0(HPMicroTarget):
    """HPM68x0 (Y=0 base) RISC-V MCU.

    Family base target with RAM only. No flash region defined.
    Y>0 variants (HPM68D2/D3/D4) extend this with internal flash.
