...
from .flash_algo_CY8C64xA import flash_algo as flash_algo_main
...
from .target_CY8C6xxA import CortexM_CY8C6xxA
...
class PSoC6AFlashSecure(Flash):
    def __init__(self, target, flash_algo):
        super(PSoC6AFlashSecure, self).__init__(target, flash_algo)
    ...
    def init(self, operation, address=None, clock=0, reset=True):
        ...
        is_flashing = True
        super(PSoC6AFlashSecure, self).init(operation, address, clock, reset)
        is_flashing = True
    ...
    def uninit(self):
        ...
        global is_flashing
        super(PSoC6AFlashSecure, self).uninit()
        is_flashing = False

class Flash_CY8C64xA_Main(PSoC6AFlashSecure):
    def __init__(self, target):
        super(Flash_CY8C64xA_Main, self).__init__(target, flash_algo_main)


class Flash_CY8C64xA_Work(PSoC6AFlashSecure):
    def __init__(self, target):
        super(Flash_CY8C64xA_Work, self).__init__(target, flash_algo_work)

        
class Flash_CY8C64xA_SMIF(PSoC6AFlashSecure):
    def __init__(self, target):
        super(Flash_CY8C64xA_SMIF, self).__init__(target, flash_algo_smif)
...
class cy8c64xA(CoreSightTarget):
    VENDOR = "Cypress"
    AP_NUM = None
    ...
    memoryMap = MemoryMap(
        ...
        FlashRegion(start=0x10000000, length=0x1D0000, blocksize=0x200,
#        FlashRegion(start=0x10000000, length=0x200000, blocksize=0x200,
                    ...
                    flash_class=Flash_CY8C64xA_Main),
        ...
        FlashRegion(start=0x14000000, length=0x8000, blocksize=0x200,
                    ...
                    erase_sector_weight=ERASE_SECTOR_WEIGHT,
                    program_page_weight=PROGRAM_PAGE_WEIGHT,
                    flash_class=Flash_CY8C64xA_Work),
        ...
        FlashRegion(start=0x18000000, length=0x4000000, blocksize=0x40000, page_size=0x1000,
                    ...
                    flash_class=Flash_CY8C64xA_SMIF),

        RamRegion(start=0x08000000, length=0x20000)
    )

    def __init__(self, link, ap_num):
        super(cy8c64xA, self).__init__(link, self.memoryMap)
        self.AP_NUM = ap_num

    def create_init_sequence(self):
        seq = super(cy8c64xA, self).create_init_sequence()
        seq.replace_task('find_aps', self.find_aps)
        seq.replace_task('create_cores', self.create_cy8c6xxa_core)
        return seq
    ...
    def create_cy8c6xxa_core(self):
        core = CortexM_CY8C64xA(self.session, self.aps[self.AP_NUM], self.memory_map, 0)
...
class CortexM_CY8C64xA(CortexM):
    ...
    def reset_and_halt(self, reset_type=None):
        ...
        with Timeout(25.0) as t_o:
            while t_o.check():
                try:
                    if self.read32(0x4022004C) == 0x12344321:
                        break
                except exceptions.TransferError:
                    pass
        ...
        try:
            if self.ap.ap_num == 2 and self.read32(0x40201200) & 3 != 3:
                LOG.debug("CM4 is sleeping, trying to wake it up...")
                self.write32(0x40201200, 0x05fa0003)
        except exceptions.TransferError:
            pass
    ...
    def resume(self):
        global is_flashing
        super(CortexM_CY8C64xA, self).resume()
...
###################################################################################################################
class cy8c64xA_cm4_full_flash(cy8c64xA):
    memoryMap = MemoryMap(
        RomRegion(start=0x00000000, length=0x20000),
    ...
    def __init__(self, link):
        super(cy8c64xA_cm4_full_flash, self).__init__(link, 2)


class CortexM_CY8C64xA_full(CortexM_CY8C6xxA):
    def reinit_dap(self):
        with Timeout(2.0) as t_o:
            while t_o.check():
                try:
...
class cy8c64xA_cm4_full(CoreSightTarget):
    VENDOR = "Cypress"
    AP_NUM = 2
    ...
    def __init__(self, link):
        super(cy8c64xA_cm4_full, self).__init__(link, self.memoryMap)

    def create_init_sequence(self):
        seq = super(cy8c64xA_cm4_full, self).create_init_sequence()
        seq.replace_task('find_aps', self.find_aps)
    ...
    def create_cy8c6xx7_core(self):
        core = CortexM_CY8C64xA_full(self.session, self.aps[self.AP_NUM], self.memory_map, 0)
        core.default_reset_type = self.ResetType.SW_SYSRESETREQ
        self.aps[self.AP_NUM].core = core
        core.init()
        self.add_core(core)
