...
class _APM32F10xCLFamily(CoreSightTarget):
    VENDOR = "Geehy"
...
MEMORY_MAP_APM32F10xCx = MemoryMap(
    FlashRegion(start=0x08000000, length=0x40000, blocksize=0x800, page_size=0x400,
        is_boot_memory=True, algo=FLASH_ALGO),
    RamRegion(start=0x20000000, length=0x10000)
)


class APM32F105xC(_APM32F10xCLFamily):
    # Covers APM32F105RC/VC
    MEMORY_MAP = MEMORY_MAP_APM32F10xCx

    def __init__(self, session):
        super().__init__(session, self.MEMORY_MAP)


class APM32F107xC(_APM32F10xCLFamily):
    # Covers APM32F107RC/VC
    MEMORY_MAP = MEMORY_MAP_APM32F10xCx
